    ...
    <div className="container mx-auto p-4 md:p-8 animate-fade-in">
      ...
      <p className="text-lg text-center text-gray-400 mb-8 max-w-2xl mx-auto">
        {hasPreviousResult 
          ...
          : "Select an effect below and upload image. 选择想实现的效果（如“3D手办效果”）并上传您的图片"
        }
      </p>
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
        {transformations.map((trans, index) => (
          ...
          >
            <span className="text-4xl mb-1 transition-transform duration-200 group-hover:scale-110">{trans.emoji}</span>
            <span className="font-semibold text-base text-gray-200 leading-tight">{trans.title}</span>
